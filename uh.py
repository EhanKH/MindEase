...
class HomeScreen(ctk.CTkFrame):
    def __init__(self, parent, on_navigate):
        ...
        self.grid_rowconfigure(2, weight=0) # Stats
        self.grid_rowconfigure(3, weight=1) # Spacer

        # Use a central container to manage width
        ...
        # --- NEW: Centered App Title ---
        ctk.CTkLabel(
            container,
            text="MINDEASE",
            font=("Inter", 64, "bold"), # Significantly larger and bolder
        ...
        # UPDATED: Simple quote directly under welcome
        quote = ctk.CTkLabel(
            hero_frame,
        ...
        self._make_stat_card(
            ...
            title="App Streak",
            value="3 days",
            desc="Used this 3 days in a row.",
        )
        self._make_stat_card(
            stats_frame,
            col=1,
            title="Mood Streak",
            value="5 days",
            desc="Used this 5 days in a row.",
        )
        self._make_stat_card(
            stats_frame,
            col=2,
            title="Last Survey Result",
            value="Moderate",
            desc="Used this 2 days ago.",
        )
        self._make_stat_card(
            stats_frame,
            col=3,
            title="Yesterday's Reports",
            value="1",
            desc="1 AI summary yesterday.",
        )
    ...
    def _make_stat_card(self, parent, col, title, value, desc):
        ...
        # FIX: Removed ipady, adjusted row spacing and padding to fit large font
        card.grid(row=0, column=col, padx=12, sticky="ew") 
        card.grid_columnconfigure(0, weight=1)
        # FIX: Explicitly ensure the third row (description) has height for wrapping
        card.grid_rowconfigure(2, minsize=80) 

        ctk.CTkLabel(
            card,
            text=title,
            font=Theme.fonts["sans_sm"],
            text_color=Theme.colors["text-secondary"],
        ...
        ctk.CTkLabel(
            ...
            font=Theme.fonts["sans_lg"],
            text_color=Theme.colors["text-primary"],
        ).grid(row=1, column=0, sticky="w", padx=20, pady=(0, 0)) 
        ...
        ctk.CTkLabel(
            ...
            anchor="w",
            justify="left",
        # FIX: Use sticky="nw" to push content to the top of the cell and adjust bottom padding
        ).grid(row=2, column=0, sticky="nw", padx=20, pady=(8, 15)) 


# ---------------------------------------------------------------------------
# SURVEY SCREEN - UPDATED WITH AI
# ---------------------------------------------------------------------------
class SurveyScreen(ctk.CTkFrame):
    def __init__(self, parent):
        super().__init__(parent, fg_color=Theme.colors["bg-main"])
        
        self.all_answers = [] # <-- To store answers for the AI

        self.questions = [
            (
                "Over the last week, how often have you felt overwhelmed?",
                ["Never", "Sometimes", "Often", "Almost Constantly"],
                [0, 1, 2, 3], # Original scores, no longer used for logic
            ),
            (
                "How would you rate your sleep quality?",
                ["Great", "Okay", "Not Good", "Terrible"],
                [0, 1, 2, 3],
            ),
            (
                "How motivated have you felt to do schoolwork?",
                ["Very Motivated", "Somewhat", "Not Really", "No Motivation"],
                [0, 1, 2, 3],
            ),
            (
                "Have you been making time for activities you enjoy?",
                ["Definitely", "A little", "Not really", "Not at all"],
                [0, 1, 2, 3],
            ),
            (
            ...
            (
                ...
                ["Very well", "Okay", "Poorly", "Very poorly"],
                [0, 1, 2, 3],
            ),
            (
        ...
        # The internal guidance data is kept for the tips but the main description will be AI-generated
        self.guidance = {
            "Low": {
                "title": "Low",
        ...
        self.total_score = 0 # No longer used for logic, but kept for consistency
        ...
        self.subtitle = ctk.CTkLabel(
            container,
            text="Answer a few quick questions for a wellbeing snapshot.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        )
        ...
        self.results_stress_label = ctk.CTkLabel(
            self.results_card,
            text="",
            font=("Inter", 32, "bold"), 
            text_color=Theme.colors["text-primary"],
        ...
        self.results_desc_label = ctk.CTkLabel(
            self.results_card,
            text="",
            font=Theme.fonts["sans_sm"],
            text_color=Theme.colors["text-secondary"],
        ...
        self.restart_btn = ctk.CTkButton(
            ...
            font=Theme.fonts["sans_btn"],
            fg_color=Theme.colors["bg-hover"],
            hover_color=Theme.colors["border-color"],
            height=55, 
    ...
    def handle_answer(self, answer_index: int):
        ...
        # Store the text of the question and the answer for the AI
    ...
    def get_ai_guidance(self, result_key: str):
        # Determine key based on AI response
        if "Low" in result_key:
            key = "Low"
        elif "High" in result_key:
            key = "High"
        else:
            # Default to Moderate if AI response is weird or an error
            key = "Moderate"

        # Start loading the description in a new thread
    ...
    def _threaded_generate_guidance(self, key: str):
        ...
        # Generate the supportive paragraph
        data_str = "\n".join([f"Q: {item['question']}\nA: {item['answer']}" for item in self.all_answers])
        prompt = (
    ...
    def show_results(self, key: str, ai_paragraph: str):
        ...
        self.results_desc_label.configure(text=ai_paragraph) # Use AI-generated paragraph
    ...
    def restart(self):
        self.current_index = 0
        self.total_score = 0
        self.all_answers = [] # <-- Reset answers
        self.render_question()


# ---------------------------------------------------------------------------
# PLANNER / TIMETABLE SCREEN - UPDATED WITH AI
# ---------------------------------------------------------------------------
class PlannerScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container,
            text="Add your tasks. The AI will help you prioritize them.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ...
        self.task_entry = ctk.CTkEntry(
            add_row,
            placeholder_text="e.g., Finish Math homework",
            font=Theme.fonts["sans_sm"],
            fg_color=Theme.colors["bg-hover"],
            border_color=Theme.colors["border-color"],
            height=55, 
        ...
        self.add_btn = ctk.CTkButton(
            ...
            font=Theme.fonts["sans_btn"],
            fg_color=Theme.colors["accent"],
            hover_color=Theme.colors["accent-hover"],
            width=100, 
            height=55, 
        ...
        self.ai_btn_error_label = ctk.CTkLabel(
            self.ai_btn_frame, 
            text="", 
            font=Theme.fonts["sans_sm"], 
        ...
        self.ai_btn = ctk.CTkButton(
            self.ai_btn_frame,
            text="Prioritize with AI", # <-- No more demo!
            font=Theme.fonts["sans_btn"],
            fg_color=Theme.colors["accent"],
            hover_color=Theme.colors["accent-hover"],
            height=55, 
            width=200, 
            command=self.prioritize_tasks,
        )
    ...
    def _create_task_row(self, index: int, text: str, priority: str | None):
        ...
        def toggle():
            if var.get():
                label.configure(
                    font=Theme.fonts["sans_sm"] + ("overstrike",),
                    text_color=Theme.colors["text-secondary"],
                )
            else:
                label.configure(
        ...
        rm_btn = ctk.CTkButton(
            ...
            height=34, 
            fg_color="transparent",
            hover_color=Theme.colors["bg-card"],
            text_color=Theme.colors["text-secondary"],
    ...
    def _finish_prioritize(self, reply: str):
        try:
            # Clean reply if it includes markdown ```json
            ...
            for task in self.tasks:
                # Check for case-insensitive matches just in case
        ...
        # Always re-render and re-enable
        self.render_tasks()
...
class RelaxScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            breathe_card,
            text="60-Second Focus Reset",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-primary"],
        ).grid(row=0, column=0, pady=(30, 15), padx=30, sticky="w") 

        circle_wrapper = ctk.CTkFrame(breathe_card, fg_color="transparent")
        circle_wrapper.grid(row=1, column=0, pady=30) 

        self.breath_circle = ctk.CTkFrame(
            circle_wrapper,
            width=180, 
            height=180, 
            fg_color=Theme.colors["accent"],
        ...
        self.breath_label = ctk.CTkLabel(
            self.breath_circle,
            text="Start",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-primary"],
...
# ---------------------------------------------------------------------------
# HELPER (CHAT) SCREEN – Updated to use new requests-based AI
# ---------------------------------------------------------------------------
class HelperScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container,
            text="A gentle, anonymous helper. It listens and offers small suggestions.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ).grid(row=1, column=0, sticky="w", pady=(0, 20)) 
        ...
        self.chat_log = ctk.CTkScrollableFrame(
            card,
            fg_color=Theme.colors["bg-sidebar"],
            border_width=1,
            border_color=Theme.colors["border-color"],
    ...
    def ensure_intro(self):
        if not self.bot_started:
            intro_msg = (
                "Hi, I'm the MindEase AI Helper. I'm here to listen. "
                "You can talk about school, stress, energy, or anything on your mind."
            )
    ...
    def _threaded_respond(self):
        """Runs in a thread to avoid freezing the UI"""
        
        # Create the full payload from history
        payload = [
...
# ---------------------------------------------------------------------------
# MODULE 1: ASSESSMENT SCREEN (Marks / Advice) - Updated with new AI
# ---------------------------------------------------------------------------
class AssessmentScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container,
            text="Add your subjects and marks (out of 100). We'll help you see where to focus.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ).grid(row=1, column=0, sticky="w", pady=(0, 20)) 
        ...
        self.result_card.grid_remove() # Hide it initially
        
        # --- ELEMENTS FOR ASSESSMENT RESULTS ---
        ...
        # Note: We're placing this directly now that the summary is removed.
        self.overall_percent_label.grid(row=0, column=0, sticky="w", padx=24, pady=(20, 10))
        
        # Divider between overall average and improvement advice
        ctk.CTkFrame(self.result_card, height=1, fg_color=Theme.colors["border-color"]).grid(
            row=1, column=0, sticky="ew", padx=24, pady=(10, 15)
        )
        
        # Improvement Advice Header
        ctk.CTkLabel(
            self.result_card,
            text="🎯 Improvement Advice",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-primary"],
            anchor="w",
        ).grid(row=2, column=0, padx=24, pady=(0, 6), sticky="w")

        # Improvement Advice Label (AI-generated)
        self.improvement_advice_label = ctk.CTkLabel(
            self.result_card,
            text="-- Your AI Improvement Advice will appear here --",
            font=Theme.fonts["sans_sm"],
            text_color=Theme.colors["text-secondary"],
            wraplength=850, 
            justify="left",
    ...
    def analyse(self):
        ...
        self.improvement_advice_label.configure(text="Getting AI Advice...") # Placeholder for loading
        
        threading.Thread(
            target=self._threaded_analyse, 
            args=(subjects_data, overall_percentage), 
            daemon=True
        ).start()

    def _threaded_analyse(self, subjects_data, overall_percentage):
        """Runs in a thread to call the AI"""
    ...
    def _show_analysis(self, reply: str):
        """Updates UI from the main thread"""
...
# ---------------------------------------------------------------------------
# MODULE 2: MOOD & ENERGY SCREEN - Updated with new AI
# ---------------------------------------------------------------------------
class MoodEnergyScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container,
            text="Quickly log how your mind and body feel right now.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ).grid(row=1, column=0, sticky="w", pady=(0, 20)) 
        ...
        self.mood_scale = ctk.CTkSlider(
            mood_frame,
            from_=1,
            to=5,
            number_of_steps=4,
            command=self._update_mood_emoji,
            progress_color=Theme.colors["accent"],
            button_color=Theme.colors["accent"],
            button_hover_color=Theme.colors["accent-hover"],
            height=12, 
        )
        self.mood_scale.set(3)
        self.mood_scale.grid(row=1, column=0, sticky="ew", pady=(16, 6)) 

        self.mood_emoji_label = ctk.CTkLabel(
            mood_frame,
            text="😐 Okay",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ...
        self.energy_scale = ctk.CTkSlider(
            energy_frame,
            from_=1,
            to=5,
            number_of_steps=4,
            command=self._update_energy_text,
            progress_color=Theme.colors["yellow"],
        ...
        ctk.CTkLabel(
            notes_card,
            text="Anything you want to note? (Optional)",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-primary"],
        ).grid(row=0, column=0, sticky="w", padx=30, pady=(20, 6)) 
    ...
    def save_checkin(self):
        ...
        threading.Thread(
            target=self._threaded_get_summary, 
            args=(mood, energy, notes), 
            daemon=True
        ).start()
...
class ReportScreen(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container,
            text="Get an AI-generated summary of your check-ins and progress.",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-secondary"],
        ).grid(row=1, column=0, sticky="w", pady=(0, 20)) 
        ...
        self.report_card.grid_remove() # Hide it

        ctk.CTkLabel(
            self.report_card,
            text="✨ Your AI-Powered Report",
            font=Theme.fonts["sans_md"],
            text_color=Theme.colors["text-primary"],
        ).grid(row=0, column=0, padx=30, pady=(20, 8), sticky="w") 
    ...
    def _threaded_get_report(self):
        """
        Runs in a thread.
        Simulates fetching and formatting data for the AI report.
        """
